...
Y = 120e3;
Z = 800; %Y,Z: dimensions of domain[m]
...
alpha = 0.0005;
G_base = 58.3e-3;
%!%!%!%!%!%!%!%!%!% Note this should be run with 4*(thermal body force) %!%!%!%!%!%!%!%!%!%
...
drivingRamp = 1;
enforcedMargin = 60e3; %enforcedMargin: margin location [m]
tau_base = -175e3*exp(-abs(y-60e3)/0.1e3)...
           -28e3*exp(-abs(y-41e3)/0.15e3)...
           -35e3*exp(-abs(y-15e3)/0.15e3)...
           -1.845e3;
...
hold on
plot((-dnB(:,1)+enforcedMargin)/1e3,dnB(:,2),'rx');
...
hold on
plot((-dnB(:,1)+60e3)/Y,dnB(:,2)/(3.1557e7*max(u(m*n-m+1:m*n))),'r.','MarkerSize',20);
axis([0 0.52 0 1.1])
