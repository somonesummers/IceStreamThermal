...
tau_base = -300e3*exp(-abs(y-52.0e3)/0.4e3)...
           -30e3*exp(-abs(y-38.0e3)/0.6e3)...
           -40e3*exp(-abs(y-28.0e3)/0.6e3)...
           -12.8e3;
...
load profileVelData2.mat
hold on
plot((-line_dist+max(line_dist))/1e3,line_speed,'rx');
xlim([0 Y/1e3])
