...
tau_base(:,i) = -40e3*exp((y-61.0e3)/1.2e3)...
                -(3.2e3+79e3*(t(i)/1e9))*exp(-abs(y-33.2e3)/(10.0e3-(t(i)/1e9)*8.3e3))...
                -2.0e3;
...
for i = 1:2
    plot(y(1:m)/1000,3.1557e7*u(m*n-m+1:m*n,i),'LineWidth',3)
end
box on
set(gca,'Layer','top')
axis([11.3 70 0 600])
...
subplot(3,1,2)
plotTemperatureField(m,nT,y,zT,T(:,1),T_m)
xlim([11.3 70])
set(gca, 'xdir','reverse')
setFontSize(16)
set(gcf,'color','w');
...
subplot(3,1,3)
plotTemperatureField(m,nT,y,zT,T(:,2),T_m)
xlim([11.3 70])
set(gca, 'xdir','reverse')
setFontSize(16)
set(gcf,'color','w');
