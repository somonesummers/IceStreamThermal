...
thermalEnhancement = 10*ones(1,legth(t)); %thermalEnhancement: enhancement of shear heating []
enforcedMargin = 30e3*ones(1,length(t)); %enforcedMargin: margin location [m]
for i = 1:length(t)
tau_base(:,i) = -40e3*exp((y-61.0e3)/1.2e3)...
                -(3.7e3+76.3e3*(t(i)/1e9)^2)*exp(-abs(y-33.2e3)/(10.0e3-(t(i)/1e9)^(1/1.3)*9.7e3))...
                -2.0e3;
...
%% %% %% %% %%     Visualization     %% %% %% %% %%
save data.mat
%See plotAnimation.m for visualization of the time-dependent simulation