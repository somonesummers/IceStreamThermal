...
Y = 40e3;
Z = 870; %Y,Z: dimensions of domain[m]
...
drivingRamp = 1;
enforcedMargin = 20e3; %enforcedMargin: margin location [m]
tau_base = -300e3*exp(-abs(y-20e3)/0.4e3)...
           -40e3*exp(-abs(y-15e3)/0.35e3)...
           -30e3*exp(-abs(y-10e3)/0.3e3)...
...
hold on
plot(-profileC(:,1)+20,profileC(:,2),'rx');
axis([0 40 0 600])
