...
Y = 35e3;
Z = 825; %Y,Z: dimensions of domain[m]
...
drivingRamp = 1;
enforcedMargin = 17.5e3; %enforcedMargin: margin location [m]
tau_base = -300e3*exp(-abs(y-18.4e3)/0.5e3)...
           -20e3*exp(-abs(y-14.5e3)/0.5e3)...
           -15e3*exp(-abs(y-11.5e3)/0.5e3)...
           -30e3*exp(-abs(y-9e3)/0.5e3)...
           -0.08e3;
...
hold on
plot(-profileD(:,1)+17.5,profileD(:,2),'rx');
axis([0 35 0 700])
