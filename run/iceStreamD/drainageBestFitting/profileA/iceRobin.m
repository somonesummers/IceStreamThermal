...
Y = 60e3;
Z = 890; %Y,Z: dimensions of domain[m]
...
drivingRamp = 1;
enforcedMargin = 30e3; %enforcedMargin: margin location [m]
tau_base = -300e3*exp(-abs(y-31e3)/0.6e3)...
           -50e3*exp(-abs(y-25.0e3)/0.4e3)...
           -45e3*exp(-abs(y-20.0e3)/0.4e3)...
           -35e3*exp(-abs(y-15.0e3)/0.35e3)...
           -25e3*exp(-abs(y-10.0e3)/0.3e3)...
           -20e3*exp(-abs(y-5.0e3)/0.3e3)...
           -3.4e3;
...
hold on
plot(-profileA(:,1)+30,profileA(:,2),'rx');
